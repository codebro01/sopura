export const teamMembers = [
  {
    id: 1,
    name: 'Rufus Oluwole',
    position: 'Director',
    image:
      'https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80',
    expertise: [
      'He is a seasoned banker with over a decade of experience cut across various industry in top managerial capacity He is a finace expert and a member of various proffesional bodies including ICAN and CIFCON',
    ],
    experience: [
      {
        role: '',
        company: 'Intecontinetal bank',
        period: '2006 - 2008',
      },
      {
        role: '',
        company: 'De –RENNY GLOBAL INTEGRATED LTD SER ',
        period: '2008 - 2017',
      },
      {
        role: '',
        company: 'Credo Consult Ltd',
        period: '2017 - 2018',
      },
      {
        role: '',
        company: 'StonelMill Nigeria Ltd',
        period: '2018 - 2022',
      },
      {
        role: '',
        company: 'Unity Bank Nigeria',
        period: '2022 - 2024',
      },
    ],
    education: [
      {
        degree: 'BSc. in Biochemistry',
        institution: 'University of Ilorin',
        year: '',
      },
    ],

    ['data-aos']: 'zoom-out-up',
    ['data-aos-anchor-placement']: 'center-bottom',
    ['data-aos-duration']: '200',
  },
  {
    id: 2,
    name: 'Abioye Oluseye',
    position: 'Director',
    image:
      'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80',
    expertise: [
      'Has over a decade of experience in the Nigerian financial services industry, ranging fromTtreasury operations, Credit risk and Private Banking Business development Financial Advisory and Consulting',
    ],
    experience: [
      {
        role: '',
        company: 'Managing Consultant at Kadoct Solutions.',
        period: '2016 - Present',
      },
      {
        role: '',
        company: 'Equitorial Trust Bank Ltd/Sterling Bank Plc',
        period: '2008 - 2016',
      },
      {
        role: '',
        company: 'Intercontinental Bank Plc',
        period: '2005 - 2008',
      },
      {
        role: '',
        company: 'Equity Bank of Nigeria',
        period: '2002 - 2005',
      },
    ],
    education: [
      {
        degree: 'BSc in Chemistry',
        institution: 'University of Ilorin',
        year: '',
      },
      {
        degree: 'MBA ',
        institution: 'University of Ilorin',
        year: '',
      },
    ],

    ['data-aos']: 'zoom-out-up',
    ['data-aos-anchor-placement']: 'center-bottom',
    ['data-aos-duration']: '500',
  },
  //   {
  //     id: 3,
  //     name: 'Alicia Rodriguez',
  //     position: 'Full Stack Developer',
  //     image:
  //       'https://images.unsplash.com/photo-1580489944761-15a19d654956?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80',
  //     expertise: ['React', 'Node.js', 'TypeScript', 'GraphQL', 'AWS'],
  //     experience: [
  //       {
  //         role: 'Full Stack Developer',
  //         company: 'TechInnovate Inc.',
  //         period: '2020 - Present',
  //       },
  //       {
  //         role: 'Frontend Engineer',
  //         company: 'Facebook',
  //         period: '2017 - 2020',
  //       },
  //       {
  //         role: 'Web Developer',
  //         company: 'Twitter',
  //         period: '2015 - 2017',
  //       },
  //     ],
  //     education: [
  //       {
  //         degree: 'M.S. in Computer Science',
  //         institution: 'University of California, Berkeley',
  //         year: '2015',
  //       },
  //       {
  //         degree: 'B.S. in Software Engineering',
  //         institution: 'Carnegie Mellon University',
  //         year: '2013',
  //       },
  //     ],
  //   },
  //   {
  //     id: 4,
  //     name: 'James Wilson',
  //     position: 'Data Science Lead',
  //     image:
  //       'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80',
  //     expertise: [
  //       'Machine Learning',
  //       'Data Analysis',
  //       'Python',
  //       'TensorFlow',
  //       'Statistical Modeling',
  //     ],
  //     experience: [
  //       {
  //         role: 'Data Science Lead',
  //         company: 'TechInnovate Inc.',
  //         period: '2019 - Present',
  //       },
  //       {
  //         role: 'Senior Data Scientist',
  //         company: 'Microsoft',
  //         period: '2016 - 2019',
  //       },
  //       {
  //         role: 'Research Scientist',
  //         company: 'IBM Research',
  //         period: '2013 - 2016',
  //       },
  //     ],
  //     education: [
  //       {
  //         degree: 'Ph.D. in Machine Learning',
  //         institution: 'University of Washington',
  //         year: '2013',
  //       },
  //       {
  //         degree: 'M.S. in Statistics',
  //         institution: 'University of Michigan',
  //         year: '2010',
  //       },
  //     ],
  //   },
  //   {
  //     id: 5,
  //     name: 'Emily Tanaka',
  //     position: 'Product Manager',
  //     image:
  //       'https://images.unsplash.com/photo-1544005313-94ddf0286df2?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80',
  //     expertise: [
  //       'Product Strategy',
  //       'Market Research',
  //       'Agile Methodologies',
  //       'User Testing',
  //     ],
  //     experience: [
  //       {
  //         role: 'Product Manager',
  //         company: 'TechInnovate Inc.',
  //         period: '2018 - Present',
  //       },
  //       {
  //         role: 'Associate Product Manager',
  //         company: 'Spotify',
  //         period: '2016 - 2018',
  //       },
  //       {
  //         role: 'Business Analyst',
  //         company: 'Amazon',
  //         period: '2014 - 2016',
  //       },
  //     ],
  //     education: [
  //       {
  //         degree: 'MBA',
  //         institution: 'Harvard Business School',
  //         year: '2014',
  //       },
  //       {
  //         degree: 'B.S. in Information Systems',
  //         institution: 'NYU',
  //         year: '2011',
  //       },
  //     ],
  //   },
  //   {
  //     id: 6,
  //     name: 'David Okafor',
  //     position: 'DevOps Engineer',
  //     image:
  //       'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=80',
  //     expertise: [
  //       'CI/CD',
  //       'Docker',
  //       'Kubernetes',
  //       'Infrastructure as Code',
  //       'Cloud Security',
  //     ],
  //     experience: [
  //       {
  //         role: 'DevOps Engineer',
  //         company: 'TechInnovate Inc.',
  //         period: '2020 - Present',
  //       },
  //       {
  //         role: 'Site Reliability Engineer',
  //         company: 'Netflix',
  //         period: '2017 - 2020',
  //       },
  //       {
  //         role: 'Systems Administrator',
  //         company: 'Salesforce',
  //         period: '2015 - 2017',
  //       },
  //     ],
  //     education: [
  //       {
  //         degree: 'M.S. in Computer Engineering',
  //         institution: 'Georgia Tech',
  //         year: '2015',
  //       },
  //       {
  //         degree: 'B.S. in Computer Science',
  //         institution: 'University of Texas at Austin',
  //         year: '2012',
  //       },
  //     ],
  //   },
]
